...
-spec find_function(woody:service_name(), woody:func(), [module()]) ->
    {module(), dmt_thrift:function_schema()} | notfound.
...
-spec get_function_schema(module(), woody:service_name(), woody:func()) -> dmt_thrift:function_schema().
...
-spec get_param_types(dmt_thrift:function_schema()) -> [dmt_thrift:thrift_type()].
get_param_types({{struct, _, SchemaDef}, _, _}) ->
    [Type || {_N, _Req, Type, _Name, _Def} <- SchemaDef].

-spec get_reply_type(dmt_thrift:function_schema()) -> dmt_thrift:thrift_type().
get_reply_type({_, ReplyType, _}) ->
    ReplyType.

-spec get_exception_type(tuple(), dmt_thrift:function_schema()) -> {Name :: atom(), dmt_thrift:thrift_type()}.
...
json_to_term_(Json, {undefined, Type}, Stack) ->
    json_to_term_(Json, {optional, Type}, Stack);
...
-spec term_to_json(term(), dmt_thrift:thrift_type()) -> jsone:json_value().
